...
public class GaussianProcess {
	...
	public double negativeLogLikelihood(Matrix logtheta, Matrix x, Matrix y, Matrix df0) {
		...
		if (!cd.isSPD()) {
			...
			Matrix W = bSubstitutionWithTranspose(L, (fSubstitution(L, Matrix.identity(n, n))))
					.minus(alpha.times(alpha.transpose())); // precompute for
															// convenience
	...
	public Matrix[] predict(Matrix xstar) {
		...
		if (xstar.getColumnDimension() != X.getColumnDimension())
			throw new IllegalArgumentException(
					"Wrong size of the input " + xstar.getColumnDimension() + " instead of " + X.getColumnDimension());
	...
	private Matrix minimize(Matrix params, int length, Matrix in, Matrix out) {
		...
		while (i < nCycles) {
			...
			while (true) { // keep extrapolating as long as necessary
				...
				x3 = x1 - d1 * (x2 - x1) * (x2 - x1) / (B + Math.sqrt(B * B - A * d1 * (x2 - x1))); // num.
																									// error
																									// possible,
																									// ok!
			...
			while ((Math.abs(d3) > -SIG * d0 || f3 > f0 + x3 * RHO * d0) && M > 0) { // keep
																						// interpolating
				...
				if (f4 > f0) {
					x3 = x2 - (0.5 * d2 * (x4 - x2) * (x4 - x2)) / (f4 - f2 - d2 * (x4 - x2)); // quadratic
																								// interpolation
				} else {
					A = 6 * (f2 - f4) / (x4 - x2) + 3 * (d4 + d2); // cubic
																	// interpolation
					B = 3 * (f4 - f2) - (2 * d2 + d4) * (x4 - x2);
					x3 = x2 + (Math.sqrt(B * B - A * d2 * (x4 - x2) * (x4 - x2)) - B) / A; // num.
																							// error
																							// possible,
																							// ok!
				}
				...
				x3 = Math.max(Math.min(x3, x4 - INT * (x4 - x2)), x2 + INT * (x4 - x2)); // don't
																							// accept
																							// too
																							// close
	...
	public static void main(String[] args) {
		...
		// plot.fillPlot();
		// }
*/		double[] temp2 = { 1, 2, 3, 4, 3, 4, 5, 76, 4, 5, 6, 7, 8, 9, 67, 5, 4, 3, 32, 22, 34, 56, 44, 33, 77, 86, 34 };
		Matrix[] datastar = ArrayToMatrix.load2(temp2, 6, 1);
